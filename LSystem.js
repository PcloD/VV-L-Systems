function Rule(_from, _to)
{
	this.left = _from; // MAPS TO...
	this.right = _to;
}

function LSystem(_alphabet, _axiom, _rules, _angle, _edgelength = 10)
{
	this.Alphabet = _alphabet;
	this.Axiom = _axiom;
	this.Rules = _rules;
	this.Angle = radians(_angle);
	...
	this.Execute01 = function(initialPosition, initialAngle)
	{
		...
		var currentAngle = radians(initialAngle);
		...
		for(var c=0; c<this.Axiom.length; c++)
		{
			switch(this.Axiom.charAt(c))
			{
				case('F'):
					...
					newPos.add(p5.Vector.fromAngle(currentAngle).mult(this.Edge));
					line(currentPosition.x, currentPosition.y, newPos.x, newPos.y);
					currentPosition = newPos;
					break;
				case('B'):
					currentPosition = currentPosition.sub(p5.Vector.fromAngle(currentAngle).mult(this.Edge));
					break;
				case('R'):
